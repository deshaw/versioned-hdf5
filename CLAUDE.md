...
See `docs/releasing.md`. Releases are cut by creating a GitHub Release with a new
tag; the `pypi_publish.yml` workflow uses PyPI trusted publishing (no maintainer
credentials). A conda-forge feedstock bot follows up automatically — verify the
feedstock's minimum-version pins still match `pyproject.toml` before merging.
