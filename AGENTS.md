...
handles) to each other. Two separate libhdf5 instances would each hold their own copy
of HDF5's global state, so an ID minted by one would be meaningless to the other →
corruption/crashes. They must share a single libhdf5. (libcrypto, used by `hash.pyx`,
is the exception and stays vendored — h5py does not ship it.)
...
be **pinned to a digest** matching the `H5PY_VERSION` in `wheels.yml`, never left on
`:latest`. Bump the digests in lockstep with `H5PY_VERSION`; pick the digest from a
green CI run's "Build wheel" log (the `Digest: sha256:...` line of the image pull).
macOS and Windows are unaffected — they build HDF5 themselves rather than borrowing it.
